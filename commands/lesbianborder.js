...
module.exports = {
    command: "lesbianborder",
    description: "makes things lesbian... but border?!?!",
    category: "Borders",
    admin: false,
    /**
     * Execute the command
    ...
    execute: async function(client, message) {
        ...
        new https.request(`https://some-random-api.ml/canvas/lesbian?avatar=${url}`, (res) => {
            var lesbianDir = path.join(__dirname, "..", "imgs", "lesbian")
            const id = crypto.randomUUID()
            var filename = path.join(lesbianDir, `${id}.png`)
            if(!fs.existsSync(lesbianDir)) {
                fs.mkdirSync(lesbianDir, {recursive: true})
            }
