...
module.exports = {
    command: "panborder",
    description: "makes things pansexual... but border?!?!",
    category: "Borders",
    admin: false,
    /**
     * Execute the command
    ...
    execute: async function(client, message) {
        ...
        new https.request(`https://some-random-api.ml/canvas/pansexual?avatar=${url}`, (res) => {
            var pansexualDir = path.join(__dirname, "..", "imgs", "pansexual")
            const id = crypto.randomUUID()
            var filename = path.join(pansexualDir, `${id}.png`)
            if(!fs.existsSync(pansexualDir)) {
                fs.mkdirSync(pansexualDir, {recursive: true})
            }
