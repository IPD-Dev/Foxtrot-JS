...
module.exports = {
    command: "jail",
    description: "dont drop the soap",
    category: "Filters",
    admin: false,
    /**
     * Execute the command
    ...
    execute: async function(client, message) {
        ...
        new https.request(`https://some-random-api.ml/canvas/jail?avatar=${url}`, (res) => {
            var jailDir = path.join(__dirname, "..", "imgs", "jail")
            const id = crypto.randomUUID()
            var filename = path.join(jailDir, `${id}.png`)
