...
module.exports = {
    command: "gayborder",
    description: "makes things gay... but border?!?!",
    category: "Borders",
    admin: false,
    /**
     * Execute the command
    ...
    execute: async function(client, message) {
        ...
        new https.request(`https://some-random-api.ml/canvas/lgbt?avatar=${url}`, (res) => {
            var lgbtDir = path.join(__dirname, "..", "imgs", "lgbt")
            const id = crypto.randomUUID()
            var filename = path.join(lgbtDir, `${id}.png`)
            if(!fs.existsSync(lgbtDir)) {
                fs.mkdirSync(lgbtDir, {recursive: true})
            }
