...
module.exports = {
    ...
    execute: async function(client, message) {
        ...
        new https.request(`https://some-random-api.ml/canvas/gay?avatar=${url}`, (res) => {
            var gayDir = path.join(__dirname, "..", "imgs", "gay")
            const id = crypto.randomUUID()
            var filename = path.join(gayDir, `${id}.png`)
            if(!fs.existsSync(gayDir)) {
                fs.mkdirSync(gayDir, {recursive: true})
            }
